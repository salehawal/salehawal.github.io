...
function arrangeblocks()
{
	...
	{
		blocks[i].innerHTML += "<h5>("+(i+1)+") | Left("+blocks[i].offsetLeft+") Top("+blocks[i].offsetTop+") Height("+blocks[i].offsetHeight+") </h5>";
		/*if(i%2 == 0)
		{
			r = i+1; // next block
			div_h_l = blocks[i].offsetHeight;
			div_l += div_h_l; // left block height
			div_h_r = (typeof blocks[r] !== 'undefined')?blocks[r].offsetHeight:0;
			div_r += div_h_r; // right block height
			direction = (div_r > div_l)?"l":"r"; // set new direction
			if(i == 0) // first time
			{
				if(div_h_r > div_h_l)
					c_diff = div_h_r - div_h_l;
			}
			else if(div_h_r == 0) // no more blocks
			{
			...
			else if(i >= 2) // new row
			{
				if(direction == "r")
					...
					a  = i;
				if(c_diff) // apply height adjustment
				{
					blocks[a].style.marginTop = "-"+c_diff.toString()+"px";
					c_diff_o = c_diff;
					c_diff = 0;
				}

				c_diff = (div_h_r > div_h_l)?(div_h_r - div_h_l):(div_h_l - div_h_r); // calc new gap size
				c_diff = (div_r > div_l)?(c_diff + c_diff_o):(Math.abs(c_diff - c_diff_o)); // sum old + new gap size
			}
		}*/
	}
}