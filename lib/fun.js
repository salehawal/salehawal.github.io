...
function genGallery(imgs, loc, id) {
  ...
  for (var i = 0; i < imgs.length; i++) {
    ...
    var isExternal = u.startsWith('http://') || u.startsWith('https://');
    
    if (isExternal) {
      // External link - keep as is
      html += '<div class="block-img"><a href="' + u + '" target="_blank"><img src="' + loc + '/' + f + '"></a></div>';
    } else {
      // Internal image - use preview modal
      html += '<div class="block-img"><img class="gallery-preview-img" src="' + u + '" alt="" style="cursor: pointer;"></div>';
    }
  }
...
function initializeImagePreview() {
  ...
  // Get all preview images (both tasks_img and gallery-preview-img)
  function attachPreviewListeners() {
    const allPreviewImages = document.querySelectorAll(".tasks_img, .gallery-preview-img");
  ...
  // Re-attach listeners when gallery is loaded (for dynamic content)
